...
export const PlantPhotoUploadModal: React.FC<PlantPhotoUploadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fetchClientError) {
      ...
      try {
        ...
        if (xhrResult.ok && xhrResult.status === 201) {
          onSuccessToast?.('Fotografía guardada con éxito (vía fallback).');
          handleClose();
          router.refresh();
          return;
        }
    ...
    } else if (fetchClientError === null && !probePassed && !errorMessage) {
      // Diagnostic if probe failed or unexpected path
