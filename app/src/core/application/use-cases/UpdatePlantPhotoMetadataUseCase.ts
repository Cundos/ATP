...
export interface UpdatePlantPhotoMetadataCommand {
  plant_id: string;
  photo_id: string;
  taken_at?: Date | null;
  caption?: string | null;
...
export class UpdatePlantPhotoMetadataUseCase {
  constructor(
    private readonly plantRepository: IPlantRepository,
    private readonly photoRepository: IPhotoRepository
  ) {}

  async execute(command: UpdatePlantPhotoMetadataCommand): Promise<PhotoEntity> {
    ...
    const trimmedCaption =
      command.caption !== undefined
        ? command.caption === null || command.caption.trim() === ''
          ? null
          : command.caption.trim()
        : undefined;

    return await this.photoRepository.updateMetadata(photo.id, {
      ...(command.taken_at !== undefined ? { taken_at: command.taken_at } : {}),
